...
class Task(object):
    def execute(self):
        ...
        print("This task in not yet configured")
        exit(1)

# Engine to manage the flow of tasks
class Engine(object):
    def _init__(self, task_map):
...
# Example subclass reperseting a failure state
class Failure(Task):
    def execute(self):
...
# Example of a more complex task with decision brances
...
class CheckRunningConfig(Task):
    def execute(self):
        ...
        if 'interface GigabitEthernet0/1' in running_config:
            ...
            print("Configuartion missing. Task failed")
            return 'failure'
