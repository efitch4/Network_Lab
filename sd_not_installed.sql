SELECT DISTINCT
    ...
    CASE 
        WHEN LOWER(sl.softwarename) = 'ManageEngine UEMS - Agent ' THEN 'Installed'
        ELSE 'Not Installed'
    END AS "Service Desk Installation Status"
FROM 
    SystemInfo si
...
WHERE 
    si.WORKSTATIONID NOT IN (
        ...
        WHERE LOWER(sl2.softwarename) = 'ManageEngine UEMS - Agent '
    )
GROUP BY 
    si.WORKSTATIONNAME, 
    state.DISPLAYSTATE, 
