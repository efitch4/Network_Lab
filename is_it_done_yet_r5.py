...
def remove_selected_from_listbox(listbox, log_textbox):
    selected_items = listbox.curselection()
    if not selected_items:
        messagebox.showinfo("No Selection", "Please select an item to remove.")
        return

    for index in selected_items[::-1]:  # Reverse to avoid index shifting issues
        item = listbox.get(index)
        listbox.delete(index)
        log_textbox.insert(tk.END, f"Removed from Complete Files: {item}\n")
        log_textbox.see(tk.END)


def create_context_menu(listbox, log_textbox):
    context_menu = tk.Menu(listbox, tearoff=0)
    context_menu.add_command(label="Remove Selected", command=lambda: remove_selected_from_listbox(listbox, log_textbox))
...
create_context_menu(complete_listbox, log_textbox)  # Attach context menu
