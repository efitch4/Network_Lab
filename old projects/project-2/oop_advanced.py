...
class ApplyConfig(Task):
    def execute(self):
        ...
        try:
            ...
            logging.info("New configuration applied to %s", device['host'])
            return 'verify_config'
        except Exception as e:
            print(f"Configuration application failed: {e}")
            logging.error("Configuration application failed with %s: %s", device['host'], e)
    ...
    def execute(self):
        ...
        try:
            ...
            if "interface GigabitEthernet0/2" in running_config:
                ...
                logging.info("New configuration verified on %s", device['host'])
                return 'save_config'
            else:
...
class SaveConfig(Task):
    def execute(self):
        ...
        try:
            ...
            logging.info("Configuration saved successfully on %s", device['host'])
            return 'complete'
        except Exception as e:
