...
# Step 2: Create a table if it does'nt exist
...
def insert_user(conn, name, age, email):
    cursor = conn.cursor()
    try:
        cursor.exexcute("INSERT INTO users (name, age, email) VALUES (?, ?, ?)", (name, age, email))
        conn.commit()
