...
def ping_host(host, results, device_status=None):
    """Pings a single host and stores the result. Prioritize TeamViewer status if available."""
    # Attempt local ping
...
def start_ping():
    ...
    # Fetch TeamViewer devices and map aliases to statuses
    devices = fetch_teamviewer_devices()
    teamviewer_status = {}
    for device in devices:
        alias = device.get('alias', 'Unknown')
        last_ip = device.get('last_ip', 'No IP')
        status = device.get('status', 'Offline')
        teamviewer_status[alias] = status
        if last_ip:
            teamviewer_status[last_ip] = status  # Include IP-based fallback
