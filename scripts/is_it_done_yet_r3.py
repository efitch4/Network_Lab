...
def move_completed_files(complete_listbox, log_textbox):
    ...
    log_textbox.insert(tk.END, f"Moving files to: {destination_folder}\n")
    log_textbox.see(tk.END)

    for idx in range(complete_listbox.size()):
        file_entry = complete_listbox.get(idx)
        if "✅" in file_entry:
            file_name = file_entry.split("✅")[-1].strip()  # Extract file path
            try:
                destination_path = os.path.join(destination_folder, os.path.basename(file_name))
                os.rename(file_name, destination_path)
                log_textbox.insert(tk.END, f"Moved: {file_name} -> {destination_path}\n")
            except Exception as e:
                log_textbox.insert(tk.END, f"Error moving {file_name}: {e}\n")
            log_textbox.see(tk.END)

    messagebox.showinfo("Operation Complete", "All completed files have been moved.")
...
move_button = tk.Button(
    progress_frame, text="Move Completed Files",
    command=lambda: move_completed_files(complete_listbox, log_textbox)
)
